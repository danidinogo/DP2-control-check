...
public interface AdministratorDashboardRepository extends AbstractRepository {
	...
	@Query("SELECT COUNT(c), COUNT(i) from Item i LEFT JOIN Xomemi c ON c.artefact.id = i.id")
	String getArtefactsWithChimpumRatio();
	
	@Query("SELECT c.ration.currency, MIN(c.ration.amount), MAX(c.ration.amount), AVG(c.ration.amount), STDDEV(c.ration.amount) from Xomemi c GROUP BY c.ration.currency")
